	...
	const float* gain_map_d, const float* map_ptr_x_d, const float* map_ptr_y_d, float* fitting_para_d, float* CRLBs_d, float* LogLikelihood_d, float* device_debug_d)
{
	...
	{
		int num_fitting_para;
		bool offset_fit = para_config->LS_os_fit;
		if (offset_fit)
			num_fitting_para = 6;
		else
			num_fitting_para = 5;
		...
		float* Maxjump = new float[num_fitting_para];// = { 1,1,12,9,1.5,10 };  x y z max value in simu/3*2   h bg max value in simu/2/3*2 
		*Maxjump = 1;
		*(Maxjump + 1) = 1;
		*(Maxjump + 2) = 12;
		*(Maxjump + 3) = 9;
		*(Maxjump + 4) = 1.5;
		...
		float* jacobian = new float[num_fitting_para];
		memset(jacobian, 0, num_fitting_para * sizeof(float));
		float* hessian = new float[num_fitting_para * num_fitting_para];
		memset(hessian, 0, num_fitting_para * num_fitting_para * sizeof(float));
		...
		float* NewDudt = new float[num_fitting_para]; //dmu/dx  dmu/dy  dmu/dz  dmu/dh  dmu/dbg dmu/dz_offset
		memset(NewDudt, 0, num_fitting_para * sizeof(float));
		float model, data;
		float NewChiSq;
		float OldChiSq = 1e11;
		...
		float* NewUpdate = new float[num_fitting_para];  //delta x y z h bg z_offset
		float* OldUpdate = new float[num_fitting_para];

		for (int i = 0; i < num_fitting_para; i++)
		{
			*(NewUpdate + i) = 1e11;
			*(OldUpdate + i) = 1e11;
		}
		...
		float* L = new float[num_fitting_para * num_fitting_para];
		float* U = new float[num_fitting_para * num_fitting_para];
		float* M = new float[num_fitting_para * num_fitting_para];
		float* Minv = new float[num_fitting_para * num_fitting_para];
		float* Diag = new float[num_fitting_para];
		memset(L, 0, num_fitting_para * num_fitting_para * sizeof(float));
		memset(U, 0, num_fitting_para * num_fitting_para * sizeof(float));
		//delete[] L, U, M, Minv, Diag;
		

		if (offset_fit) // initialize parameters to be fitted
		{
		...
		{
			//initialize light sheet offset using fitting results of fiduciary 
			{
				kernel_bg_eval(data_cur, offset_map_d, gain_map_d, &pos_x, &pos_y, NewTheta, para_config->num_slice, para_config->cam_x);
				kernel_h_bg_init(data_cur, offset_map_d, gain_map_d, &pos_x, &pos_y, NewTheta, para_config->num_slice, para_config->cam_x);
				kernel_xy_init(data_cur, offset_map_d, gain_map_d, &pos_x, &pos_y, NewTheta, para_config->num_slice, para_config->cam_x);
		...
		for (int ii = 0; ii < num_fitting_para; ii++) OldTheta[ii] = NewTheta[ii];
		...
		{  // calculate for each pixel. initialize alpha beta and ChiSq
			...
			for (int k = 0; k < num_fitting_para; k++) jacobian[k] -= (1 - data / model) * NewDudt[k];

			for (int j = 0; j < num_fitting_para; j++) for (int k = j; k < num_fitting_para; k++)
			{
				hessian[j * num_fitting_para + k] += data / model / model * NewDudt[j] * NewDudt[k];
				hessian[k * num_fitting_para + j] = hessian[j * num_fitting_para + k];
			}
		...
		for (iter = 0; iter < iterations; iter++)
		{
			...
			else
			{
				...
				for (int i = 0; i < num_fitting_para; i++)
				{
					hessian[i * num_fitting_para + i] = hessian[i * num_fitting_para + i] * mu;
				}
				memset(L, 0, num_fitting_para * num_fitting_para * sizeof(float));
				memset(U, 0, num_fitting_para * num_fitting_para * sizeof(float));
				errFlag = kernel_cholesky(hessian, num_fitting_para, L, U);
				if (errFlag == 0)
				{
					for (int i = 0; i < num_fitting_para; i++)
					{
					...
					OldLambda = NewLambda;
					OldChiSq = NewChiSq;
					kernel_luEvaluate(L, U, jacobian, num_fitting_para, NewUpdate);
					//updateFitParameters
					for (int ll = 0; ll < num_fitting_para; ll++)
					{
		...
		{  // re-evaluate alpha beta
			...
			for (int j = 0; j < num_fitting_para; j++) for (int k = j; k < num_fitting_para; k++)
			{
				M[j * num_fitting_para + k] += NewDudt[j] * NewDudt[k] / model * 4;
				M[k * num_fitting_para + j] = M[j * num_fitting_para + k];
			}
			if ((data > 0) && (model > 0))
			{
		...
		*(LogLikelihood_d + idx) = ChiSq_min;
		kernel_MatInvN(M, Minv, Diag, num_fitting_para);
